...
public class BoardGateway implements board.BoardConstants {
	...
	public BoardGateway(TextArea textArea, VBox storypane) {
		...
		try {
			...
			// Platform.runLater(() -> storypane.appendText("Exception in gateway
	...
	public int getCommentCount() {
		...
		try {
			...
			// Platform.runLater(() -> storypane.appendText("Error in getCommentCount: " +
			// ex.toString() + "\n"));
		}
		return count;
	}
	...
	public int getStoryCount() {
		...
		try {
			...
			// Platform.runLater(() -> storypane.appendText("Error in getStoryCount: " +
			// ex.toString() + "\n"));
		}
		return count;
	}
	...
	public String getComment(int n) {
		...
		try {
			...
			// Platform.runLater(() -> storypane.appendText("Error in getComment: " +
			// ex.toString() + "\n"));
		}
		return comment;
	}
	...
	public int getLastDeleted() {
		...
		int n = 0;
		try {
			n = Integer.parseInt(inputFromServer.readLine());
		} catch (IOException ex) {
			Platform.runLater(() -> textArea.appendText("Error in getStory: " + ex.toString() + "\n"));
			// Platform.runLater(() -> storypane.appendText("Error in getStory: " +
			// ex.toString() + "\n"));
